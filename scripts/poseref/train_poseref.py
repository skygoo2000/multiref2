...
# Add torch profiler imports for memory monitoring
...
from videox_fun.data.dataset_image_video import (ImageVideoRefDataset,
                                                ImageVideoSampler,
                                                get_random_mask)
from videox_fun.models import (AutoencoderKLWan, AutoencoderKLWan3_8, WanT5EncoderModel, MultiRefTransformer3DModel)
from videox_fun.pipeline import WanFunPoseRefPipeline
from videox_fun.utils.discrete_sampler import DiscreteSampling
from videox_fun.utils.fm_solvers_unipc import FlowUniPCMultistepScheduler
from videox_fun.utils.utils import get_image_to_video_latent, save_videos_grid, get_image_latent, padding_image
    ...
    try:
        ...
        for i in range(len(args.validation_prompts)):
            with torch.no_grad():
                        ...
                        if ref_ext in ['jpg', 'jpeg', 'png', 'bmp', 'tiff', 'webp']:
                            ...
                            try:
                                ...
                                for idx in frame_indices:
                                    ...
                                    # Resize and pad the frame to match validation size
                ...
                # Load validation pose if provided
                validation_ref_pose = None
                validation_video_pose = None
                if args.validation_pose_path is not None:
                    pose_path = args.validation_pose_path
                    logger.info(f"Loading validation pose from: {pose_path}")
                    try:
                        from videox_fun.data.dataset_image_video import pose_dict_to_matrix
                        ...
                        if 'video' in pose_data:
                            total_video_poses = len(pose_data['video'])
                            if total_video_poses <= val_frames:
                                video_pose_indices = list(range(total_video_poses))
                            else:
                                video_pose_indices = np.linspace(0, total_video_poses - 1, val_frames, dtype=int).tolist()
                            
                            video_pose_list = []
                            for idx in video_pose_indices:
                                pose_matrix = pose_dict_to_matrix(pose_data['video'][idx])
                                video_pose_list.append(pose_matrix)
                            
                            if video_pose_list:
                                validation_video_pose = np.stack(video_pose_list, axis=0)  # [F, 4, 4]
                                validation_video_pose = torch.from_numpy(validation_video_pose).unsqueeze(0)  # [1, F, 4, 4]
                                logger.info(f"Loaded validation video_pose with shape: {validation_video_pose.shape}")
                        
                        # Process ref pose
                        if 'ref' in pose_data:
                            total_ref_poses = len(pose_data['ref'])
                            if total_ref_poses <= num_ref_frames_in_vid:
                                ref_pose_indices = list(range(total_ref_poses))
                            else:
                                # Same logic as dataset: always take first frame
                                ref_pose_indices = [0]
                                if num_ref_frames_in_vid > 1:
                                    remaining_indices = np.linspace(1, total_ref_poses - 1, num_ref_frames_in_vid - 1, dtype=int)
                                    ref_pose_indices.extend(remaining_indices.tolist())
                            
                            ref_pose_list = []
                            for idx in ref_pose_indices:
                                pose_matrix = pose_dict_to_matrix(pose_data['ref'][idx])
                                ref_pose_list.append(pose_matrix)
                            
                            if ref_pose_list:
                                validation_ref_pose = np.stack(ref_pose_list, axis=0)  # [F, 4, 4]
                                validation_ref_pose = torch.from_numpy(validation_ref_pose).unsqueeze(0)  # [1, F, 4, 4]
                                logger.info(f"Loaded validation ref_pose with shape: {validation_ref_pose.shape}")
                    
                    except Exception as e:
                        logger.warning(f"Failed to load validation pose: {e}")
                        validation_ref_pose = None
                        validation_video_pose = None
                ...
                if validation_ref is not None:
                    ...
                    ref_pose_input = validation_ref_pose.to(device=accelerator.device, dtype=weight_dtype) if validation_ref_pose is not None else None
                    video_pose_input = validation_video_pose.to(device=accelerator.device, dtype=weight_dtype) if validation_video_pose is not None else None
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("Path to pose JSON file used for validation."),
    )
    parser.add_argument(
        "--validation_size",
        nargs=3,
    ...
    parser.add_argument(
        "--enable_profiler", 
        action="store_true", 
        help="Enable torch profiler for memory and performance monitoring."
    )
    parser.add_argument(
        "--profiler_schedule_wait",
        type=int,
        default=1,
        help="Number of steps to wait before profiling starts."
    )
    parser.add_argument(
...
def main():
    ...
    # Get Transformer
    # For Wan2.1, there's no separate high/low noise models, load directly from main path
    ...
    transformer_path = os.path.join(args.pretrained_model_name_or_path, sub_path)
    if not os.path.exists(os.path.join(transformer_path, "config.json")):
        # Wan2.1 style: transformer config is in the main directory
    ...
    transformer3d = MultiRefTransformer3DModel.from_pretrained(
        transformer_path,
    ...
    if args.transformer_path is not None:
        ...
        if missing_pos_emb or missing_projector:
            print("### Initializing missing pos_emb and/or projector with zero/identity initialization in resume checkpoint")
        ...
        # RefTransformer3DModel uses the same weights as the base model, no additional parameters
        if len(u) > 0:
    ...
    if args.use_ema:
        ...
        ema_transformer3d = MultiRefTransformer3DModel.from_pretrained(
            os.path.join(args.pretrained_model_name_or_path, sub_path),
            transformer_additional_kwargs=transformer_additional_kwargs,
        ).to(weight_dtype)

        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=MultiRefTransformer3DModel, model_config=ema_transformer3d.config)
        ...
        if fsdp_stage != 0:
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    _, ema_kwargs = MultiRefTransformer3DModel.load_config(ema_path, return_unused_kwargs=True)
                    load_model = MultiRefTransformer3DModel.from_pretrained(
                        input_dir, subfolder="transformer_ema",
                        transformer_additional_kwargs=transformer_additional_kwargs
                    )
                    load_model = EMAModel(load_model.parameters(), model_cls=MultiRefTransformer3DModel, model_config=load_model.config)
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            new_examples["fg"] = []
            # Used for pose
            new_examples["ref_pose"] = []
            new_examples["video_pose"] = []
            ...
            for example in examples:
                ...
                # Process pose if exists
                if "ref_pose" in example and example["ref_pose"] is not None:
                    new_examples["ref_pose"].append(example["ref_pose"])
                else:
                    new_examples["ref_pose"].append(None)
                
                if "video_pose" in example and example["video_pose"] is not None:
                    new_examples["video_pose"].append(example["video_pose"])
                else:
                    new_examples["video_pose"].append(None)
            ...
            # Stack pose data if they exist
            if all(example is not None for example in new_examples["ref_pose"]):
                # ref_pose: list of [F, 4, 4] numpy arrays -> [B, F, 4, 4]
                new_examples["ref_pose"] = np.stack(new_examples["ref_pose"], axis=0)
            else:
                new_examples.pop("ref_pose")
            
            if all(example is not None for example in new_examples["video_pose"]):
                # video_pose: list of [F, 4, 4] numpy arrays -> [B, F, 4, 4]
                # Crop to batch_video_length
                cropped_video_poses = []
                for pose in new_examples["video_pose"]:
                    cropped_video_poses.append(pose[:batch_video_length])
                new_examples["video_pose"] = np.stack(cropped_video_poses, axis=0)
            else:
                new_examples.pop("video_pose")
    ...
    if args.enable_profiler and accelerator.is_main_process:
        ...
        print(f"Profiler enabled with schedule: wait={args.profiler_schedule_wait}, warmup={args.profiler_schedule_warmup}, active={args.profiler_schedule_active}, repeat={args.profiler_schedule_repeat}")
    
    # Create a conditional context manager for profiler recording
    def conditional_record_function(name):
        """Return record_function context if profiler is enabled, otherwise return a no-op context"""
    ...
    def log_memory_usage(step_name, global_step):
        """Log current memory usage to tensorboard"""
        if args.enable_profiler and torch.cuda.is_available() and accelerator.is_main_process:
    ...
    # Define sample_ref_frames function for training loop
    def sample_ref_frames(total_frames, num_ref_frames_to_use=6, rng=None):
        """
        Sample reference frames using the same logic as dataset.
    ...
    with profiler_context:
        for epoch in range(first_epoch, args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if epoch == first_epoch and step == 0:
                    pixel_values, ref_pixel_values, texts = batch['pixel_values'].cpu(), batch['ref_pixel_values'].cpu(), batch['text']
                    pixel_values = rearrange(pixel_values, "b f c h w -> b c f h w")
                    ref_pixel_values = rearrange(ref_pixel_values, "b f c h w -> b c f h w")
                    os.makedirs(os.path.join(args.output_dir, "sanity_check"), exist_ok=True)
                    for idx, (pixel_value, ref_pixel_value, text) in enumerate(zip(pixel_values, ref_pixel_values, texts)):
                    ...
                    # Log pose info if available
                    if 'ref_pose' in batch and batch['ref_pose'] is not None:
                        ref_pose = batch['ref_pose']
                        logger.info(f"ref_pose shape: {ref_pose.shape}, dtype: {ref_pose.dtype}")
                    if 'video_pose' in batch and batch['video_pose'] is not None:
                        video_pose = batch['video_pose']
                        logger.info(f"video_pose shape: {video_pose.shape}, dtype: {video_pose.dtype}")
                    if args.train_mode != "normal":
                ...
                # Initialize num_ref_frames_to_use for progress bar display
                num_ref_frames_to_use = None
                ...
                with accelerator.accumulate(transformer3d):
                    ...
                    ref_batch_indices = []  # Store indices for ref_pose sampling
                    ...
                    for ref in sampled_ref_pixel_values:
                        ...
                        if ref_frames < max_ref_frames:
                            ...
                            cycled_ref = ref
                        aligned_ref_pixel_values.append(cycled_ref)
                    ref_pixel_values = torch.stack(aligned_ref_pixel_values)  # [B, F, C, H, W]
                    ...
                    ref_pose = batch.get("ref_pose", None)  # [B, F, 4, 4] numpy array
                    video_pose = batch.get("video_pose", None)  # [B, F, 4, 4] numpy array
                    
                    # Sample ref_pose using same indices as ref_pixel_values
                    if ref_pose is not None:
                        # Convert to torch first
                        ref_pose = torch.from_numpy(ref_pose).to(accelerator.device, dtype=weight_dtype)  # [B, F, 4, 4]
                        # Sample ref_pose for each sample in batch using same indices as ref_pixel_values
                        sampled_ref_poses = []
                        for b in range(batch_size):
                            # Use the same ref_batch_index that was used for ref_pixel_values
                            sampled_ref_poses.append(ref_pose[b, ref_batch_indices[b]])
                        
                        # Align ref_poses using the SAME max_ref_frames as ref_pixel_values
                        # This ensures ref_pixel_values and ref_pose have the same frame dimension
                        aligned_ref_poses = []
                        for i, pose in enumerate(sampled_ref_poses):
                            pose_frames = pose.size(0)
                            ref_frames = sampled_ref_pixel_values[i].size(0)
                            
                            # Ensure pose_frames matches ref_frames (they should be the same after sampling)
                            if pose_frames != ref_frames:
                                raise ValueError(f"Mismatch: ref_pixel_values[{i}] has {ref_frames} frames but ref_pose[{i}] has {pose_frames} frames after sampling")
                            
                            # Use the same max_ref_frames and alignment logic as ref_pixel_values
                            if pose_frames < max_ref_frames:
                                repeat_times = max_ref_frames // pose_frames
                                remainder = max_ref_frames % pose_frames
                                if remainder > 0:
                                    cycled_pose = torch.cat([pose.repeat(repeat_times, 1, 1), pose[:remainder]], dim=0)
                                else:
                                    cycled_pose = pose.repeat(repeat_times, 1, 1)
                            else:
                                cycled_pose = pose
                            aligned_ref_poses.append(cycled_pose)
                        ref_pose = torch.stack(aligned_ref_poses)  # [B, F, 4, 4]
                        
                        # Final verification: ensure ref_pixel_values and ref_pose have the same frame dimension
                        if ref_pixel_values.shape[1] != ref_pose.shape[1]:
                            raise ValueError(f"Final alignment mismatch: ref_pixel_values has {ref_pixel_values.shape[1]} frames but ref_pose has {ref_pose.shape[1]} frames")
                    
                    if video_pose is not None:
                        video_pose = torch.from_numpy(video_pose).to(accelerator.device, dtype=weight_dtype)
                    ...
                    # Process ref latents as full_ref
                    # ref_latents shape: [B, C, F, H, W]
                    # RefTransformer3DModel can handle both single frame [B, C, H, W] and multi-frame [B, C, F, H, W]
                    ...
                    if args.enable_text_encoder_in_dataloader:
                        ...
                        with torch.no_grad():
                            with conditional_record_function("text_encode"):
                                prompt_ids = tokenizer(
                                    batch['text'], 
                                    padding="max_length", 
                                    max_length=args.tokenizer_max_length, 
                        ...
                        log_memory_usage("text_encode", global_step)
                    ...
                    with conditional_record_function("transformer_forward"), torch.amp.autocast("cuda", dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                        noise_pred = transformer3d(
                            ...
                            subject_ref=full_ref if args.train_mode == "normal" else None,
                            ref_pose=ref_pose,
                            video_pose=video_pose,
                        )
                    ...
                    with conditional_record_function("loss_computation"):
                        loss = custom_mse_loss(noise_pred.float(), target.float(), weighting.float())
                        loss = loss.mean()

                        # Add ref_loss based on bg_mask_downsampled region
                ...
                if num_ref_frames_to_use is not None:
                    logs["num_ref"] = num_ref_frames_to_use
        ...
        if profiler is not None and accelerator.is_main_process:
            print("Profiler finished. Trace files saved to:", logging_dir)
