        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                with torch.no_grad():
                    ...
                    # Encode fg as control_latents with random dropout
                    control_latents = None
                    if batch.get("fg") is not None:
                        fg = batch["fg"].to(weight_dtype)
                        control_latents = _batch_encode_vae(fg)
                        
                        # Apply random dropout: 10% chance to replace with zeros
                        if rng is None:
                    ...
                    if control_latents is None:
                        control_latents = mask
                    else:
                        control_latents = torch.cat([control_latents, mask], dim=1)
                ...
                if args.validation_prompts is not None and (global_step % args.validation_steps == 0 or args.max_train_steps - global_step == 1) and not args.use_fsdp:
                    ...
                    if accelerator.is_main_process:
                        ...
                        if global_step == 0:
                            ...
                            # Build comparison list: [gt, gt_decoded, full_ref_decoded, fg_decoded]
