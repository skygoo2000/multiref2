...
class Wan2_2MultiRefPipeline(DiffusionPipeline):
    r"""
    Pipeline for text-to-video generation using Wan with reference video/image support.
    ...
    def __init__(
        ...
        transformer: Wan2_2RefTransformer3DModel,
        transformer_2: Wan2_2RefTransformer3DModel = None,
    ...
    def prepare_ref_latents(
        self, ref_video, batch_size, height, width, dtype, device, generator, do_classifier_free_guidance
    ):
        """Prepare reference video/image latents"""
        if ref_video is not None:
            ref_video = ref_video.to(device=device, dtype=self.vae.dtype)
            bs = 1
            new_ref_latents = []
            for i in range(0, ref_video.shape[0], bs):
                ref_video_bs = ref_video[i : i + bs]
                ref_latents_bs = self.vae.encode(ref_video_bs)[0]
                ref_latents_bs = ref_latents_bs.mode()
                new_ref_latents.append(ref_latents_bs)
            ref_latents = torch.cat(new_ref_latents, dim=0)
        else:
            ref_latents = None
    ...
    def __call__(
        ...
        ref_video: Union[torch.FloatTensor] = None,  # New: ref video/image input
        num_frames: int = 49,
        ...
        r"""
        Function invoked when calling the pipeline for generation with reference video/image support.

        Args:
            ref_video: Reference video or image tensor of shape [B, C, F, H, W] or [B, C, H, W]
                Used as reference input for the generation process.
        ...
        if ref_video is not None:
            ...
            else:  # Video [B, C, F, H, W]
                ref_video_length = ref_video.shape[2]
                
            ref_video_processed = self.image_processor.preprocess(
                rearrange(ref_video, "b c f h w -> (b f) c h w"), height=height, width=width
            )
            ref_video_processed = ref_video_processed.to(dtype=torch.float32)
            ref_video_processed = rearrange(ref_video_processed, "(b f) c h w -> b c f h w", f=ref_video_length)
            
            ref_latents = self.prepare_ref_latents(
                ref_video_processed,
                batch_size,
                height,
                width,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                in_prompt_embeds = negative_prompt_embeds + prompt_embeds + prompt_embeds if do_classifier_free_guidance else prompt_embeds
                ...
                if ref_latents is not None:
                    ...
                    if do_classifier_free_guidance:
                        ref_latents_neg = torch.zeros_like(full_ref_single)
                        full_ref = torch.cat([ref_latents_neg, ref_latents_neg, full_ref_single])
                        full_ref = full_ref.to(device, weight_dtype)
                    else:
                        full_ref = full_ref_single.to(device, weight_dtype)
                ...
                with torch.amp.autocast('cuda', dtype=weight_dtype), torch.cuda.device(device=device):
                    noise_pred = local_transformer(
                        ...
                        seq_len=seq_len,
                        y=y,  # For inpainting
                        full_ref=full_ref,  # For reference
                    )

                # perform guidance
                if do_classifier_free_guidance:
                    noise_pred_neg, noise_pred_pos_t, noise_pred_pos_it = noise_pred.chunk(3)
                    ...
                    if t >= boundary * self.scheduler.config.num_train_timesteps:
                        noise_pred = noise_pred_neg + guide_scale_ref * (noise_pred_pos_it - noise_pred_pos_t) + guide_scale_text * (noise_pred_pos_t - noise_pred_neg)
                    else:
                        noise_pred = noise_pred_neg + guide_scale_ref * (noise_pred_pos_it - noise_pred_pos_t) + (guide_scale_text - 2.0) * (noise_pred_pos_t - noise_pred_neg)

                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]

                if self.vae.spatial_compression_ratio >= 16 and init_video is not None and not mask[:, :, 0, :, :].any():
                    latents = (1 - mask) * masked_video_latents + mask * latents
