...
from ..models import (AutoencoderKLWan, AutoTokenizer, CLIPModel,
                              WanT5EncoderModel, MultiRefTransformer3DModel)
...
class WanFunPoseRefPipeline(DiffusionPipeline):
    r"""
    Pipeline for text-to-video generation using Wan with pose reference support.
    ...
    def __init__(
        ...
        transformer: MultiRefTransformer3DModel,
    ...
    def __call__(
        ...
        fg: Optional[torch.FloatTensor] = None,
        ref_pose: Optional[torch.FloatTensor] = None,
        video_pose: Optional[torch.FloatTensor] = None,
        num_frames: int = 49,
        ...
        Args:
            ...
            subject_ref_images (`torch.FloatTensor`, *optional*):
                Reference images for subject guidance. Shape: [B, C, F, H, W] or [B, C, H, W].
            bg_mask (`torch.FloatTensor`, *optional*):
                Background mask for compositing. Shape: [B, F, C, H, W].
            fg (`torch.FloatTensor`, *optional*):
                Foreground images for compositing. Shape: [B, F, C, H, W].
            ref_pose (`torch.FloatTensor`, *optional*):
                Reference pose as homogeneous transformation matrices. Shape: [B, F, 4, 4].
            video_pose (`torch.FloatTensor`, *optional*):
                Video pose as homogeneous transformation matrices. Shape: [B, F, 4, 4].
        ...
        # Process pose data
        # ref_pose: [B, F, 4, 4] - reference pose matrices
        # video_pose: [B, F, 4, 4] - video pose matrices
        if ref_pose is not None:
            ref_pose = ref_pose.to(device=device, dtype=weight_dtype)
            logger.info(f"Pipeline received ref_pose with shape: {ref_pose.shape}")
        
        if video_pose is not None:
            video_pose = video_pose.to(device=device, dtype=weight_dtype)
            logger.info(f"Pipeline received video_pose with shape: {video_pose.shape}")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Prepare pose for CFG
                if do_classifier_free_guidance:
                    # Duplicate pose for unconditional and conditional
                    ref_pose_input = torch.cat([torch.zeros_like(ref_pose), ref_pose], dim=0) if ref_pose is not None else None
                    video_pose_input = torch.cat([torch.zeros_like(video_pose), video_pose], dim=0) if video_pose is not None else None
                else:
                    ref_pose_input = ref_pose
                    video_pose_input = video_pose
